...
- [ ] [**Memory Management (ARC)**](./Memory&Lifecycle/Memory_Management.md) (Reference Counting, Strong/Weak/Unowned의 실체)
...
## Concurrency & Threading (동시성과 흐름 제어)
...
## Swift Type System & Syntax (언어의 코어)
...
## UI & Rendering Pipeline (화면이 그려지기까지)
...
## Data, Network & Communication (데이터 흐름과 통신)
- [ ] **Design Patterns (Communication)** (Singleton Pattern의 양날의 검, Delegate Pattern의 이벤트 처리)
- [ ] **Communication Mechanisms** (NotificationCenter vs KVO vs Delegate 통신 방식 비교)
...
## Architecture & Engineering (설계와 품질 보증)
- [ ] **SOLID Principles** (객체지향 설계 5원칙의 Swift 실무 적용)
- [ ] **Architecture Patterns** (MVC의 Massive VC 문제 해결, MVVM의 ViewModel 역할과 Data Binding)
